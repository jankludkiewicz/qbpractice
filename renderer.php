...
class mod_qpractice_renderer extends plugin_renderer_base {
    ...
    public function summary_table($sessionid) {
        ...
        $table->size = array('', '');
        $table->data = array();
		$score = round($session->totalnoofquestionsright/$session->totalnoofquestions*100,2);
        $table->data[] = array($session->totalnoofquestions, $score.'% ('.$session->totalnoofquestionsright.' correct out of '.$session->totalnoofquestions.')');
    ...
    public function report_table($cm, $context) {
        ...
        if ($session != null) {
            ...
            $table->caption = get_string('pastsessions', 'qpractice');
            $table->head = array(get_string('practicedate', 'qpractice'), get_string('category', 'qpractice'), get_string('score', 'qpractice'));
            ...
            foreach ($session as $qpractice) {
				...
				$score = round($qpractice->totalnoofquestionsright/$qpractice->totalnoofquestions*100,2);
                $table->data[] = array(userdate($date), $category[$categoryid], $score.'% ('.$qpractice->totalnoofquestionsright.' correct out of '.$qpractice->totalnoofquestions.')');
            }
            echo html_writer::table($table);
        } else {
